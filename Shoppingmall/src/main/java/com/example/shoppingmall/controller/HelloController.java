...
//@Controller 선언을 해주면 HelloController 클래스는 컨트롤을 담당한다고 spring boot는 인식
//일방통행 구조 (페이지 전환(변경))
@Controller
public class HelloController {
    ...
    public String account(@RequestParam("id1")String id, Model model) {
        //클라이언트에서 받아온 id1변수 임의의 데이터를 RequestParam의 데이터로 연산작업
        //작업한 데이터를 model에 넣어서 클라이언트에 전송
        //model.addAttribute(key, value) //value에는 배열, 객체도 아무거나 넣을 수 있음
